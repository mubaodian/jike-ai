...
### HomePage.vue
- 渐变背景：蓝紫粉青渐变 + 8s 循环动画，网格叠加效果
- 输入框：无边框设计，`a-input` + `button` 分离，白色圆角容器
- 副标语改为："一句话轻松创建网站应用"
- 新增 4 个快捷提示词示例按钮（个人博客、产品展示页、电商首页、在线教育平台），点击填充输入框

### AppGeneratePage.vue
- 左右宽度比例调整为 2:3（对话 40% / 预览 60%）
- 减小全局间距（gap、padding、消息间距），页面更紧凑
- 字体缩小：header 16px、消息内容 13px
- Markdown 样式间距整体缩小约 20%
...
- 将 `HomePage.vue` 中两处完全相同的应用卡片模板抽象为独立组件
- Props：`app: API.AppVO`
- Emits：`view-chat(appId)`、`view-work(deployKey)`
- 包含卡片封面、hover 覆盖层按钮、底部用户头像 + 应用信息

**`src/components/AppDetailModal.vue`**
- 将 `AppGeneratePage.vue` 中的应用详情弹窗抽象为独立组件
- Props：`visible`、`appUserInfo`、`createTime`、`isOwnApp`、`isAdmin`
- Emits：`update:visible`、`edit`、`delete`
- 包含创建者信息、创建时间、仅本人/管理员可见的操作栏

### 页面清理

**`HomePage.vue`**
- 两处重复的卡片列表（共 ~120 行模板）替换为 `<AppCard>` 组件
- 移除未使用的 `goToAppDetail` 函数
- 移除未使用的 `.card-actions` CSS 类
- 移除 `deleteApp` import（该页面未实际使用）
- 卡片相关样式（约 90 行）迁移至 `AppCard.vue`

**`AppGeneratePage.vue`**
- 内联应用详情 Modal（47 行模板 + 9 个样式规则）替换为 `<AppDetailModal>` 组件
